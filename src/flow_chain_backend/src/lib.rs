...
fn get_client_active_orders(client_id: u64) -> Vec<Order> {
    ORDERS.with(|orders| {
        orders
            ...
            .iter()
            .filter_map(|(_, order)| {
                if order.company_id == client_id && order.order_status != "Active" {
                    Some(order.clone())
                } else {
                    None
                }
...
fn get_supplier_active_orders(supplier_id: u64) -> Vec<Order> {
    ORDERS.with(|orders| {
        orders
            ...
            .iter()
            .filter_map(|(_, order)| {
                if order.supplier_id == Some(supplier_id.to_string()) && order.order_status != "Active" {
                    Some(order.clone())
                } else {
                    None
                }
...
fn get_supplier_new_orders(supplier_id: u64) -> Vec<Order> {
    ORDERS.with(|orders| {
        orders
            ...
            .iter()
            .filter_map(|(_, order)| {
                if order.supplier_id == Some(supplier_id.to_string()) {
                    Some(order.clone())
                } else {
                    None
                }
...
fn get_driver_active_orders(driver_id: u64) -> Vec<Order> {
    ORDERS.with(|orders| {
        orders
            ...
            .iter()
            .filter_map(|(_, order)| {
                if order.driver_id == Some(driver_id.to_string()) && order.order_status != "Active" {
                    Some(order.clone())
                } else {
                    None
                }
...
#[ic_cdk::update]
async fn create_payment_reservation(amount: u64) -> Result<PaymentReservation, Error> {
    let caller = caller();
    let now = ic_cdk::api::time();

    if amount == 0 {
        return Err(Error::InvalidInput {
            msg: "Payment amount must be greater than 0".to_string(),
        });
    }
    ...
    let reservation = PaymentReservation {
        ...
        payer: caller,
        amount,
        expires_at: now + PAYMENT_RESERVATION_PERIOD,
    };
    ...
    PAYMENT_RESERVATIONS.with(|reservations| {
        ...
        let reservation = reservations
            ...
            .ok_or_else(|| Error::NotFound {
                msg: format!("Payment reservation with id={} not found", reservation_id),
            })?;

        if reservation.payer != caller {
            return Err(Error::Unauthorized {
                msg: "Only the payer can verify the payment".to_string(),
            });
        }

        if reservation.expires_at < ic_cdk::api::time() {
            return Err(Error::ValidationError {
                msg: "Payment reservation has expired".to_string(),
            });
        }

        // Here you would add actual ledger verification logic
        // For now, we'll just move it to completed payments
        COMPLETED_PAYMENTS.with(|completed| {
            completed
                .borrow_mut()
...
enum ClientCreationResult {
    Ok(Client),
    Err(String),
}

#[derive(CandidType, Debug, Serialize, Deserialize)]
enum SupplierCreationResult {
    Ok(Supplier),
    Err(String),
}
...
enum OrderCreationResult {
    ...
    // Initialize PocketIc environment
    fn setup_test_env() -> (PocketIc, Principal, CanisterId) {
        let mut ic = PocketIc::new();
        ...
        // Install your compiled Wasm
        let wasm_bytes = std::fs::read("/home/collins/devs/rust/flowchain-test/target/wasm32-unknown-unknown/release/flowchain_test_backend.wasm").expect("Failed to the read Wasm file");
        ic.install_canister(canister_id, wasm_bytes, vec![], None);

        (ic, test_principal, canister_id)
    }

    // Simulate a client creation
    fn create_test_client(ic: &PocketIc, canister_id: CanisterId, caller: Principal) -> Client {
        let client_payload = ClientCompanyPayload {
            name: "Test Client".to_string(),
            bussiness_type: "Test Type".to_string(),
            ...
            reg_no: "TEST123".to_string(),
            logo: "test_logo.png".to_string(),
        };
       ...
       let result = ic
        .update_call(
            ...
            Encode!(&client_payload).unwrap(),
        )
        .expect("Failed to call `create_client_company`");

    let client_result = if let WasmResult::Reply(bytes) = result {
        println!("Received data: {:?}", bytes);
        // Decode the result as a Result<Client, String>
        Decode!(&bytes, ClientCreationResult).map_err(|e| format!("Failed to decode ClientCreationResult: {}", e))
    } else {
        panic!("Unexpected WasmResult: {:?}", result);
    };

    match client_result {
        Ok(ClientCreationResult::Ok(client)) => client,
        Ok(ClientCreationResult::Err(err)) => panic!("Error creating client: {}", err),
        Err(e) => panic!("Failed to decode client creation result: {}", e),
    }
    }

    // Simulate a supplier creation
   fn create_test_supplier(ic: &PocketIc, canister_id: CanisterId, caller: Principal) -> Supplier {
    let supplier_payload = SupplierCompanyPayload {
        name: "Test Supplier".to_string(),
        bussiness_type: "Test Type".to_string(),
        address: "Test Address".to_string(),
        email: "supplier@test.com".to_string(),
        phone: "0987654321".to_string(),
        website: "supplier-test.com".to_string(),
        owner_name: "Test Supplier Owner".to_string(),
        reg_no: "SUP123".to_string(),
        logo: "supplier_logo.png".to_string(),
        supply_chain_type: "Test Supply Chain Type".to_string(),
    };
    ...
    let result = ic
        .update_call(
            ...
            Encode!(&supplier_payload).unwrap(),
        )
        .expect("Failed to call `create_supplier_company`");

    let supplier_result = if let WasmResult::Reply(bytes) = result {
        println!("Received data: {:?}", bytes);
        // Decode the result as a Result<SupplierCreationResult, String>
        Decode!(&bytes, SupplierCreationResult).map_err(|e| format!("Failed to decode SupplierCreationResult: {}", e))
    } else {
        panic!("Unexpected WasmResult: {:?}", result);
    };

    match supplier_result {
        Ok(SupplierCreationResult::Ok(supplier)) => supplier,
        Ok(SupplierCreationResult::Err(err)) => panic!("Error creating supplier: {}", err),
        Err(e) => panic!("Failed to decode supplier creation result: {}", e),
    }
}

    // Simulate an order creation
    fn create_test_order(ic: &PocketIc, canister_id: CanisterId, caller: Principal, client_id: u64) -> Order {
    let order_payload = OrderDetailsPayload {
        order_name: "Test Order".to_string(),
        company_id: client_id, // Ensure this is a String
        expected_delivery: "2024-12-31".to_string(),
        pickup_address: "Pickup Test".to_string(),
        delivery_address: "Delivery Test".to_string(),
        order_type: "Standard".to_string(),
        order_weight: "100kg".to_string(),
    ...
    let result = ic
        .update_call(
            ...
            caller,
            "create_order",
            Encode!(&order_payload).unwrap(),
        )
        .expect("Failed to call `create_order`");

    let order_result = if let WasmResult::Reply(bytes) = result {
        println!("Received data: {:?}", bytes);
        // Decode the result as a Result<OrderCreationResult, String>
        Decode!(&bytes, OrderCreationResult).map_err(|e| format!("Failed to decode OrderCreationResult: {}", e))
    } else {
        panic!("Unexpected WasmResult: {:?}", result);
    };

    match order_result {
        Ok(OrderCreationResult::Ok(order)) => order,
        Ok(OrderCreationResult::Err(err)) => panic!("Error creating order: {}", err),
        Err(e) => panic!("Failed to decode order creation result: {}", e),
    }
}

    #[test]
    fn test_create_bid() {
        let (mut ic, caller, canister_id) = setup_test_env();

        let client = create_test_client(&ic, canister_id, caller);
        let supplier = create_test_supplier(&ic, canister_id, caller);
        let order = create_test_order(&ic, canister_id, caller, client.id);

        println!("Order: {:?}", order); 

        let bid_payload = BidPayload {
            order_id: 2 as u64,
            amount: 1000,
            delivery_time: "2024-01-15".to_string(),
            notes: "Test bid".to_string(),
        };
        ...
        let result = ic 
            .update_call(
                ...
                caller,
                "create_bid",
                Encode!(&bid_payload).unwrap(),
            )
            .expect("Failed to call `create_bid`");


        let bid = if let WasmResult::Reply(bytes) = result {
            let bid: Bid = Decode!(&bytes, Bid).expect("Failed to decode `Bid`");
            bid
        } else {
            panic!("Unexpected WasmResult: {:?}", result);
        };

        assert_eq!(bid.supplier_id, supplier.id);
        assert_eq!(bid.order_id, order.id);
        assert_eq!(bid.amount, 1000);
        assert_eq!(bid.status, BidStatus::Pending);
    }

    #[test]
    fn test_get_order_bids() {
        let (mut ic, caller, canister_id) = setup_test_env();
        ...
        // Create bids
        let bid_payload1 = BidPayload {
            order_id: 2 as u64,
            amount: 1000,
            delivery_time: "2024-01-15".to_string(),
            notes: "Test bid 1".to_string(),
        };

        let bid_payload2 = BidPayload {
            order_id: 2 as u64,
            amount: 1200,
            delivery_time: "2024-01-14".to_string(),
            notes: "Test bid 2".to_string(),
        };
        ...
        ic.update_call(
            ...
            caller,
            "create_bid",
            Encode!(&bid_payload2).unwrap(),
        )
        .expect("Failed to call `create_bid`");

        let result = ic
            .query_call(canister_id, caller, "get_order_bids", Encode!(&order.id).unwrap())
            .expect("Failed to call `get_order_bids`");

        let bids: Vec<Bid> = if let WasmResult::Reply(bytes) = result {
            Decode!(&bytes, Vec<Bid>).expect("Failed to decode bids")
        } else {
            panic!("Unexpected WasmResult: {:?}", result);
        };
